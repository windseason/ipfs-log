...
var Log = function (_GSet) {
  ...
  function Log(ipfs, id, entries, heads, clock, key) {
    ...
    entries = entries || [];
    _this._entryIndex = entries.reduce(function (res, val) {
      res[val.hash] = val;
      return res;
    }, {}
    ...
    _this._headsIndex = heads.reduce(function (res, val) {
      res[val.hash] = val;
      return res;
    }, {}
    ...
    // Set the clock
    var maxTime = Math.max(clock ? clock.time : 0, _this.heads.reduce(function (res, acc) {
  ...
  (0, _createClass3.default)(Log, [{
    ...
    value: async function append(data) {
      ...
      var newTime = Math.max(this.clock.time, this.heads.reduce(function (res, acc) {
        return Math.max(res, acc.clock.time);
      }, 0)) + 1;
      this._clock = new Clock(this.clock.id, newTime);
      // Create the entry and add it to the internal cache
      var entry = await Entry.create(this._storage, this.id, data, (0, _values2.default)(this._headsIndex), this.clock, this._key);
    ...
    value: async function join(log) {
      ...
      ();var verifyEntries = async function verifyEntries(entries) {
        ...
        var pubkeys = _this2._keys.map(function (e) {
          return e.getPublic ? e.getPublic('hex') : e;
        });

        var checked = await pMap(entries, async function (entry) {
          if (!entry.key) throw new Error("Entry doesn't have a public key");
          if (!entry.sig) throw new Error("Entry doesn't have a signature");

          if (_this2._keys.length === 1 && _this2._keys[0] === _this2._key) {
            if (entry.id !== _this2.id) throw new Error("Entry doesn't belong in this log (wrong ID)");
          }

          if (_this2._keys.length > 0 && !_this2._keys.includes('*') && !checkAllKeys(_this2._keys.concat([_this2._key]), entry)) {
            console.warn("Warning: Input log contains entries that are not allowed in this log. Logs weren't joined.");
            return false;
          }

          try {
            await Entry.verifyEntry(entry, _this2._keystore);
          } catch (e) {
            console.log(e);
        ...
        return checked.every(function (e) {
          return e === true;
        });
      };

      var arrayOfEntriesToObject = function arrayOfEntriesToObject(res, val) {
        res[val.hash] = val;
        return res;
      };
      ...
      var difference = function difference(log, exclude) {
        ...
        while (stack.length > 0) {
          ...
          if (entry && !exclude.get(hash)) {
            ...
            entry.next.forEach(function (hash) {
      ...
      // Merge the heads
      var mergedHeads = Log.findHeads((0, _values2.default)((0, _assign2.default)({}, this._headsIndex, log._headsIndex)));
      this._headsIndex = mergedHeads.reduce(arrayOfEntriesToObject, {}

      // Find the latest clock from the heads
      );var maxClock = (0, _values2.default)(this._headsIndex).reduce(function (res, val) {
        return res = Math.max(res, val.clock.time);
      }, 0);
    ...
    value: function toJSON() {
      return {
        id: this.id,
        heads: this.heads.map(function (e) {
          return e.hash;
        })
      };
    }
    ...
    value: function toString(payloadMapper) {
      ...
      return this.values.slice().reverse().map(function (e, idx) {
        var parents = Entry.findChildren(e, _this3.values);
