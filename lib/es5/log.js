...
function (_GSet) {
  ...
  (0, _createClass2.default)(Log, [{
    ...
    value: function traverse(rootEntries) {
      ...
      var stack = rootEntries.sort(LastWriteWins).reverse(); // Cache for checking if we've processed an entry already
      ...
      var addToStack = function addToStack(entry) {
        ...
        stack = [entry].concat((0, _toConsumableArray2.default)(stack)).sort(LastWriteWins).reverse(); // Add to the cache of processed entries
