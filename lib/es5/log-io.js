...
var LogIO = function () {
  ...
  (0, _createClass3.default)(LogIO, null, [{
    ...
    value: function fromJSON(ipfs, json) {
      ...
      var mapper = function mapper(e, idx) {
        return Entry.create(ipfs, keystore, e.id, e.payload, e.next, e.clock, e.key).then(function (entry) {
    ...
    value: function expandFrom(ipfs, log, entries) {
      ...
      var hashes = entries.map(function (e) {
        return e.next;
      }).filter(function (e) {
        return e.length > 0;
      }
      ...
      // If we don't have tails, we can't expand anymore, return
        ...
        return _promise2.default.resolve({ values: log.values });
      }

      return EntryIO.fetchParallel(ipfs, hashes, amount, log.values).then(function (entries) {
        // Cap the length of the new collection (current length + wanted size)
        // const size = amount > -1 ? (log.length + amount) : -1
        var newEntries = log.merge(entries.slice(0, amount));
    ...
    value: function expand(ipfs, log) {
      ...
      return EntryIO.fetchParallel(ipfs, log.tailHashes, amount, log.values).then(function (entries) {
        // Cap the length of the new collection (current length + wanted size)
        var size = amount > -1 ? log.values.length + amount : -1;

        // Join the fetched entries with the log to order them first
        var combined = log.values.concat(entries).sort(Entry.compare);
        var sliced = size > -1 ? combined.slice(-size) : combined.slice();
        ...
        var missingOldEntries = difference(sliced, log.values, 'hash').sort(Entry.compare);
        var withoutOldEntries = difference(log.values, sliced, 'hash').sort(Entry.compare);
        var entryIntersection = intersection(log.values, sliced, 'hash').sort(Entry.compare

        // Calculate how many entries we keep from the remaining new entries
        );var length = size - (entryIntersection.length + missingOldEntries.length);
        var remainingNewEntries = length > -1 ? withoutOldEntries.slice(-length) : withoutOldEntries;
        ...
        var merged = merge(missingOldEntries, merge(entryIntersection, remainingNewEntries));
        return {
          values: merged
        };
      });
