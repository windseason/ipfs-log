...
apis.forEach((IPFS) => {
  const [identity, acl] = [getTestIdentity(), getTestACL()]
  const [id1, acl1] = [getTestIdentity('A'), getTestACL('A')]
  const [id2, acl2] = [getTestIdentity('B'), getTestACL('B')]
  const [id3, acl3] = [getTestIdentity('C'), getTestACL('C')]
  const joinPermissionCheckingFn = defaultJoinPermissionCheckingFn(['A', 'B', 'C', 'D'])
  const [joinID1, joinACL1] = [getTestIdentity('A'), getTestACL('A', joinPermissionCheckingFn)]
  const [joinID2, joinACL2] = [getTestIdentity('B'), getTestACL('B', joinPermissionCheckingFn)]
  const [joinID3, joinACL3] = [getTestIdentity('C'), getTestACL('C', joinPermissionCheckingFn)]
  const [joinID4, joinACL4] = [getTestIdentity('D'), getTestACL('D', joinPermissionCheckingFn)]
  ...
  describe('Log', function() {
    ...
    describe('constructor', async () => {
      it('creates an empty log with default params', () => {
        const log = new Log(ipfs, null, null, null, null, acl, identity)
      ...
      it('sets an id', () => {
        const log = new Log(ipfs, 'ABC', null, null, null, acl, identity)
        assert.equal(log.id, 'ABC')
      })

      it('sets the clock id', () => {
        const log = new Log(ipfs, 'ABC', null, null, null, getTestACL('XXX'), getTestIdentity('XXX'))
        assert.equal(log.id, 'ABC')
      ...
      it('throws an error if entries is not an array', () => {
        let err
        try {
          const log = new Log(ipfs, 'A', {}, null, null, acl, identity)
        } catch(e) {
          err = e
        }
      ...
      it('throws an error if heads is not an array', () => {
        let err
        try {
          const log = new Log(ipfs, 'A', [], {}, null, acl, identity)
        } catch(e) {
          err = e
        }
        assert.notEqual(err, undefined)
      ...
      it('throws an error if ACL and Identity are not defined', () => {
        ...
        try {
          const log = new Log(ipfs, null, null, null, null, acl)
        } catch(e) {
          err = e
        }
        assert.notEqual(err, undefined)
    ...
    describe('toString', async () => {
      ...
      beforeEach(async () => {
        log = new Log(ipfs, 'A', null, null, null, acl1, id1)
    ...
    describe('get', async () => {
      ...
      beforeEach(async () => {
        log = new Log(ipfs, 'AAA', null, null, null, acl, identity)
        await log.append('one')
      })
    ...
    describe('serialize', async () => {
      ...
      describe('toMultihash', async () => {
        it('returns the log as ipfs hash', async () => {
          ...
          let log = new Log(ipfs, 'A', null, null, null, acl1, id1)
          await log.append('one')
        ...
        it('throws an error if log items is empty', () => {
          const emptyLog = new Log(ipfs, null, null, null, null, acl, identity)
          let err
          try {
      ...
      describe('fromMultihash', async () => {
        it('creates a log from ipfs hash - one entry', async () => {
          ...
          const res = await Log.fromMultihash(ipfs, hash, -1, null, acl1, id1)
        ...
        it('has the right sequence number after creation and appending', async () => {
          const hash = await log.toMultihash()
          // fromMultihash length = -1
          let res = await Log.fromMultihash(ipfs, hash, -1, null, acl, identity)
          assert.equal(res.length, 3)
        ...
        it('creates a log from ipfs hash that has three heads', async () => {
          let log1 = new Log(ipfs, 'A', null, null, null, acl1, id1)
          let log2 = new Log(ipfs, 'B', null, null, null, acl2, id2)
          let log3 = new Log(ipfs, 'C', null, null, null, acl3, id3)
          ...
          // fromMultihash length = -1
          const res = await Log.fromMultihash(ipfs, hash, -1, null, acl, identity)
          assert.equal(res.length, 3)
        ...
        it('creates a log from ipfs hash up to a size limit', async () => {
          ...
          const hash = await log.toMultihash()
          const res = await Log.fromMultihash(ipfs, hash, size, null, acl, identity)
    ...
    describe('values', () => {
      it('returns all entries in the log', async () => {
        let log = new Log(ipfs, null, null, null, null, acl, identity)
    ...
    describe('join', () => {
      ...
      beforeEach(async () => {
        log1 = new Log(ipfs, 'X', null, null, null, joinACL1, joinID1)
        log2 = new Log(ipfs, 'X', null, null, null, joinACL2, joinID2)
        log3 = new Log(ipfs, 'X', null, null, null, joinACL3, joinID3)
        log4 = new Log(ipfs, 'X', null, null, null, joinACL4, joinID4)
      })
      ...
      it('joins logs', async () => {
        ...
        const logA = await Log.fromEntry(ipfs, last(items2), -1, null, joinACL3, joinID3)
        ...
        const logB = await Log.fromEntry(ipfs, last(items3), -1, null, joinACL3, joinID3)
    ...
    describe('fromEntry', () => {
      it('creates a log from an entry', async () => {
        ...
        let log = await Log.fromEntry(ipfs, data.heads, -1, null, acl1, id1)
      ...
      it('keeps the original heads', async () => {
        ...
        let log1 = await Log.fromEntry(ipfs, data.heads, data.heads.length, null, acl1, id1)
        ...
        let log2 = await Log.fromEntry(ipfs, data.heads, 4, null, acl1, id1)
        ...
        let log3 = await Log.fromEntry(ipfs, data.heads, 7, null, acl1, id1)
      ...
      it('onProgress callback is fired for each entry', async () => {
        ...
        const a = await Log.fromEntry(ipfs, last(items1), -1, [], acl1, id1, callback)
      })

      it('retrieves partial log from an entry hash', async () => {
        const log1 = new Log(ipfs, 'X', null, null, null, acl1, id1)
        const log2 = new Log(ipfs, 'X', null, null, null, acl2, id2)
        const log3 = new Log(ipfs, 'X', null, null, null, acl3, id3)
        let items1 = []
        let items2 = []
        let items3 = []
        const amount = 100
        ...
        // limit to 10 entries
        const a = await Log.fromEntry(ipfs, last(items1), 10, null, acl, identity)
        assert.equal(a.length, 10)

        // limit to 42 entries
        const b = await Log.fromEntry(ipfs, last(items1), 42, null, acl, identity)
      ...
      describe('fetches a log', () => {
        ...
        beforeEach(async () => {
          log1 = new Log(ipfs, 'X', null, null, null, acl1, id1)
          log2 = new Log(ipfs, 'X', null, null, null, acl2, id2)
          log3 = new Log(ipfs, 'X', null, null, null, acl3, id3)
          items1 = []
          items2 = []
          items3 = []
          for(let i = 1; i <= amount; i ++) {
        ...
        it('returns all entries - no excluded entries', async () => {
          const a = await Log.fromEntry(ipfs, last(items1), -1, null, acl, identity)
        ...
        it('returns all entries - including excluded entries', async () => {
          // One entry
          const a = await Log.fromEntry(ipfs, last(items1), -1, [items1[0]], acl, identity)
          assert.equal(a.length, amount)
          assert.equal(a.values[0].hash, items1[0].hash)

          // All entries
          const b = await Log.fromEntry(ipfs, last(items1), -1, items1, acl, identity)
      ...
      it('retrieves full log from an entry hash', async () => {
        ...
        const a = await Log.fromEntry(ipfs, [last(items1)], amount, null, acl1, id1)
        assert.equal(a.length, amount)

        const b = await Log.fromEntry(ipfs, [last(items2)], amount * 2, null, acl2, id2)
        assert.equal(b.length, amount * 2)

        const c = await Log.fromEntry(ipfs, [last(items3)], amount * 3, null, acl3, id3)
      ...
      it('retrieves full log from an entry hash 3', async () => {
        let log1 = new Log(ipfs, 'X', null, null, null, acl1, id1)
        ...
        const a = await Log.fromEntry(ipfs, last(items1), amount, null, joinACL1, joinID1)
        assert.equal(a.length, amount)
        ...
        const b = await Log.fromEntry(ipfs, last(items2), amount * 2, null, joinACL2, joinID2)
        assert.equal(b.length, amount * 2)
        assert.deepEqual(itemsInB, b.values.map((e) => e.payload))

        let c = await Log.fromEntry(ipfs, last(items3), amount * 3, null, joinACL3, joinID3)
        await c.append('EOF')
        ...
        let logX = new Log(ipfs, 'X', null, null, null, joinACL4, joinID4) // make sure logX comes after A, B and C
        await logX.append('1')
        await logX.append('2')
        await logX.append('3')
        const d = await Log.fromEntry(ipfs, last(logX.values), -1, null, joinACL4, joinID4)
        ...
        await c.append('DONE')
        await d.append('DONE')
        const f = await Log.fromEntry(ipfs, last(c.values), -1, [], joinACL4, joinID4)
        const g = await Log.fromEntry(ipfs, last(d.values), -1, [], joinACL4, joinID4)
      ...
      it('retrieves full log of randomly joined log', async () => {
        let log1 = new Log(ipfs, 'X', null, null, null, joinACL1, joinID1)
        let log2 = new Log(ipfs, 'X', null, null, null, joinACL2, joinID2)
        let log3 = new Log(ipfs, 'X', null, null, null, joinACL3, joinID3)
      ...
      it('retrieves randomly joined log deterministically', async () => {
        let logA = new Log(ipfs, 'X', null, null, null, joinACL1, joinID1)
        let logB = new Log(ipfs, 'X', null, null, null, joinACL2, joinID2)
        let log3 = new Log(ipfs, 'X', null, null, null, joinACL3, joinID3)
        let log  = new Log(ipfs, 'X', null, null, null, joinACL4, joinID4)
      ...
      it('retrieves partially joined log deterministically - single next pointer', async () => {
        ...
        // First 5
        let res = await Log.fromMultihash(ipfs, mh, 5, null, joinACL4, joinID4)
        ...
        // First 11
        res = await Log.fromMultihash(ipfs, mh, 11, null, joinACL4, joinID4)
        ...
        // All but one
        res = await Log.fromMultihash(ipfs, mh, 16 - 1, null, joinACL4, joinID4)
    ...
    describe('heads', () => {
      ...
      it('finds head after a join and append', async () => {
        let log1 = new Log(ipfs, 'A', null, null, null, acl1, id1)
        let log2 = new Log(ipfs, 'A', null, null, null, acl1, id1)
      ...
      it('finds two heads after three joins', async () => {
        let log1 = new Log(ipfs, 'A', null, null, null, acl1, id1)
        let log2 = new Log(ipfs, 'A', null, null, null, acl1, id1)
        let log3 = new Log(ipfs, 'A', null, null, null, acl1, id1)
    ...
    describe('tails', () => {
      ...
      it('returns tails after loading a partial log', async () => {
        ...
        await log2.append('helloB2')
        await log1.join(log2)
        const log4 = await Log.fromEntry(ipfs, log1.heads, 2, null, acl1, id1)
      ...
      it('returns tails sorted by id', async () => {
        const tailPermissionCheckingFn = defaultJoinPermissionCheckingFn(['A', 'B', 'C', 'D', 'X'])
        let log1 = new Log(ipfs, 'XX', null, null, null, getTestACL('X', tailPermissionCheckingFn), getTestIdentity('X'))
        let log2 = new Log(ipfs, 'XX', null, null, null, getTestACL('B', tailPermissionCheckingFn), getTestIdentity('B'))
        let log3 = new Log(ipfs, 'XX', null, null, null, getTestACL('A', tailPermissionCheckingFn), getTestIdentity('A'))
        let log4 = new Log(ipfs, 'XX', null, null, null, getTestACL('D', tailPermissionCheckingFn), getTestIdentity('D'))
    ...
    describe('is a CRDT', () => {
      ...
      it('multiple joins are commutative', async () => {
        ...
        // a + c == c + a
        log1 = new Log(ipfs, 'A', null, null, null, joinACL1, joinID1)
        log3 = new Log(ipfs, 'A', null, null, null, joinACL3, joinID3)
        await log1.append('helloA1')
        await log1.append('helloA2')
        await log3.append('helloC1')
        await log3.append('helloC2')
