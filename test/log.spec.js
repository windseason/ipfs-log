  ...
  describe('Log', function() {
    ...
    describe('constructor', async () => {
      ...
      it('sets the clock id', () => {
        const log = new Log(ipfs, testACL, testIdentity, 'ABC')
        assert.equal(log.id, 'ABC')
        assert.equal(log.clock.id, testIdentity.publicKey)
      })
    ...
    describe('get', async () => {
      ...
      it('returns an Entry', () => {
        ...
        assert.deepEqual(entry.hash, 'QmRLC9EW9ywLqSXMtWSt5BixRaN8w9kqqgzvDpxmsh8j9U')
      })
    ...
    describe('has', async () => {
      ...
      before(async () => {
        expectedData = {
          hash: 'QmRLC9EW9ywLqSXMtWSt5BixRaN8w9kqqgzvDpxmsh8j9U',
          id: 'AAA',
          payload: 'one',
          next: [],
          v: 0,
          clock: { id: testIdentity.publicKey,   time: 1 },
    ...
    describe('serialize', async () => {
      ...
      const expectedData = {
        id: 'AAA',
        heads: ['QmXrwXRPSQb7rEsZ6Wt8iGfsUMHPSxeguheA39NyuPnGN6']
      }
      ...
      describe('toSnapshot', () => {
        const expectedData = {
          ...
          heads: ['QmXrwXRPSQb7rEsZ6Wt8iGfsUMHPSxeguheA39NyuPnGN6'],
          values: [
            'QmRLC9EW9ywLqSXMtWSt5BixRaN8w9kqqgzvDpxmsh8j9U',
            'QmZCzPQjoYEo9fwuNTKe6Z3qsREuuDFdMeFhkhfkA2kvCX',
            'QmXrwXRPSQb7rEsZ6Wt8iGfsUMHPSxeguheA39NyuPnGN6',
          ]
        }
      ...
      describe('toMultihash', async () => {
        it('returns the log as ipfs hash', async () => {
          const expectedHash = 'QmQ43cNzJVV6a9hhHWZLQJDUhx4tmwnnySGK7zFqa31H5X'
        ...
        it('log serialized to ipfs contains the correct data', async () => {
          const expectedData = {
            id: 'A',
            heads: ['QmdFEJgfsJrJ3MQUwm1qW5CD129a7jnday6bxJzdHBLu5H']
          }
          const expectedHash = 'QmQ43cNzJVV6a9hhHWZLQJDUhx4tmwnnySGK7zFqa31H5X'
          let log = new Log(ipfs, testACL, testIdentity, 'A')
          await log.append('one')
      ...
      describe('fromMultihash', async () => {
        it('creates a log from ipfs hash - one entry', async () => {
          const expectedData = {
            id: 'X',
            heads: ['QmdB1fRJFJW4pLxZFC2mHs8r5LbeA23TtaYxYQwiZiBSyf']
          }
          ...
          assert.equal(res.values[0].clock.id, testIdentity.publicKey)
        ...
        it('creates a log from ipfs hash that has three heads', async () => {
          ...
          assert.equal(res.heads[1].payload, 'two')  // order is determined by the identity's publicKey
        ...
        it('onProgress callback is fired for each entry', async () => {
          ...
          try {
            const hash = await log.toMultihash()
            const res = await Log.fromMultihash(ipfs, testACL, testIdentity, hash, -1, [], callback)
          } catch (e) {
            throw e
          }
        })
    ...
    describe('join', () => {
      ...
      it('joins logs and updates clocks', async () => {
        ...
        assert.equal(log1.clock.id, testIdentity.publicKey)
        assert.equal(log2.clock.id, testIdentity2.publicKey)
        assert.equal(log1.clock.time, 2)
        assert.equal(log2.clock.time, 2)

        await log3.join(log1)
        assert.equal(log3.id, 'X')
        assert.equal(log3.clock.id, testIdentity3.publicKey)
        ...
        await log4.join(log1)
        assert.deepEqual(log4.clock.id, testIdentity4.publicKey)
        ...
        const expectedData = [
          { payload: 'helloA1', id: 'X', clock: { id: testIdentity.publicKey, time: 1} },
          { payload: 'helloD1', id: 'X', clock: { id: testIdentity4.publicKey, time: 1} },
          { payload: 'helloB1', id: 'X', clock: { id: testIdentity2.publicKey, time: 1} },
          { payload: 'helloA2', id: 'X', clock: { id: testIdentity.publicKey, time: 2} },
          { payload: 'helloD2', id: 'X', clock: { id: testIdentity4.publicKey, time: 2} },
          { payload: 'helloB2', id: 'X', clock: { id: testIdentity2.publicKey, time: 2} },
          { payload: 'helloC1', id: 'X', clock: { id: testIdentity3.publicKey, time: 3} },
          { payload: 'helloC2', id: 'X', clock: { id: testIdentity3.publicKey, time: 4} },
          { payload: 'helloD3', id: 'X', clock: { id: testIdentity4.publicKey, time: 5} },
          { payload: 'helloD4', id: 'X', clock: { id: testIdentity4.publicKey, time: 6} },
          { payload: 'helloA5', id: 'X', clock: { id: testIdentity.publicKey, time: 7} },
          { payload: 'helloD5', id: 'X', clock: { id: testIdentity4.publicKey, time: 7} },
          { payload: 'helloD6', id: 'X', clock: { id: testIdentity4.publicKey, time: 8} },
        ]
      ...
      it('joins logs from 4 logs', async () => {
        ...
        assert.equal(log4.clock.id, testIdentity4.publicKey)
    ...
    describe('fromEntry', () => {
      ...
      it('creates a log from an entry', async () => {
        let fixture = await LogCreator.createLog1(ipfs, testACL, identities)
      ...
      it('onProgress callback is fired for each entry', async () => {
        ...
        const a = await Log.fromEntry(ipfs, testACL, testIdentity, last(items1), -1, [], callback)
      })
      ...
      it('sorts', async () => {
        let testLog = await LogCreator.createLog1(ipfs, testACL, identities)
        let log = testLog.log
      ...
      it('sorts entries correctly', async () => {
        let testLog = await LogCreator.createLog100_2(ipfs, testACL, identities)
        let log = testLog.log
        const expectedData = testLog.expectedData
        assert.deepEqual(log.values.map(e => e.payload), expectedData)
      })
    ...
    describe('tails', () => {
      ...
      it('returns tails sorted by id', async () => {
        ...
        assert.equal(log4.tails[0].clock.id, testIdentity.publicKey)
        assert.equal(log4.tails[1].clock.id, testIdentity3.publicKey)
        assert.equal(log4.tails[2].clock.id, testIdentity2.publicKey)
        assert.equal(log4.clock.id, testIdentity4.publicKey)
      })
    })
