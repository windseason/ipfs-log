...
Object.keys(testAPIs).forEach((IPFS) => {
  describe('Log - Heads and Tails (' + IPFS + ')', function () {
    ...
    before(async () => {
      ...
      testIdentity4 = await identities.createIdentity({ id: 'userD', signingKeystore })
    ...
    describe('heads', () => {
      ...
      it('finds head after a join and append', async () => {
        let log1 = new Log(ipfs, testIdentity, identities, { logId: 'A' })
        let log2 = new Log(ipfs, testIdentity, identities, { logId: 'A' })
      ...
      it('finds two heads after three joins', async () => {
        let log1 = new Log(ipfs, testIdentity, identities, { logId: 'A' })
        let log2 = new Log(ipfs, testIdentity, identities, { logId: 'A' })
        let log3 = new Log(ipfs, testIdentity, identities, { logId: 'A' })
    ...
    describe('tails', () => {
      ...
      it('returns tails after loading a partial log', async () => {
        ...
        await log2.append('helloB2')
        await log1.join(log2)
        const log4 = await Log.fromEntry(ipfs, testIdentity, identities, log1.heads, { length: 2 })
      ...
      it('returns tails sorted by public key', async () => {
        let log1 = new Log(ipfs, testIdentity, identities, { logId: 'XX' })
        let log2 = new Log(ipfs, testIdentity2, identities, { logId: 'XX' })
        let log3 = new Log(ipfs, testIdentity3, identities, { logId: 'XX' })
        let log4 = new Log(ipfs, testIdentity4, identities, { logId: 'XX' })
