...
Object.keys(testAPIs).forEach(IPFS => {
  describe.only('Entry - Persistency (' + IPFS + ')', function () {
    ...
    it('log with one entry', async () => {
      ...
      const cid = (await log.values)[0].cid
    ...
    it('log with 2 entries', async () => {
      ...
      const cid = last(await log.values).cid
      ...
      for (let i = 1; i <= count; i++) {
        ...
        if (i % 10 === 0) {
          log2 = new Log(ipfs, testIdentity, {
            logId: log2.id,
            entries: await log2.values,
            heads: (await log2.heads).concat(await log.heads)
          })
    ...
    it('load only 99 entries from a log with 100 entries', async () => {
      ...
      for (let i = 1; i <= count; i++) {
        await log.append('hello' + i)
        if (i % 10 === 0) {
          log2 = new Log(ipfs, testIdentity, { logId: log2.id, entries: await log2.values })
          await log2.append('hi' + i)
          await log2.join(log)
        }
      }
      ...
      for (let i = 1; i <= count; i++) {
        ...
        if (i % 25 === 0) {
          log3 = new Log(ipfs, testIdentity, {
            logId: log3.id,
            entries: await log3.values,
            heads: (await log3.heads).concat(await log2.heads)
          })
          await log3.append('--' + i)
        }
      }
    ...
    it.only('load only 10 entries and then expand to max from a log with 100 entries', async () => {
      ...
      const values3 = (await log3.values).map(e => e.payload)
      const values4 = (await log4.values).map(e => e.payload)
