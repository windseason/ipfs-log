...
const { isDefined, findUniques, difference, toIpldNode, fromIpldNode } = require('./utils')
...
class LogIO {
  static async toMultihash (ipfs, log) {
    ...
    const dagNode = await toIpldNode(logData, IPLD_LINKS)
  ...
  static async fromMultihash (ipfs, hash, length = -1, exclude, onProgressCallback) {
    ...
    const logData = fromIpldNode(result.value, IPLD_LINKS)
